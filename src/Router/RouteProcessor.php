...
class RouteProcessor
{
    ...
    public function call_route_hook()
    {
        ...
        if ( !$this->matched_route->is_rest() && $this->matched_route->get_middleware() !== 'open' ) {
            ...
            return call_user_func_array( '\RubicoTechBlog\Http\Middleware\Middleware::'.$this->matched_route->get_middleware(), array($this->router) );
        }
    ...
    {
        $routes = apply_filters('ll_blog_plugin_routes', $this->routes);
        ...
        if ($routes_hash != get_option('ll_blog_plugin_routes_hash')) {
            flush_rewrite_rules();
            update_option('ll_blog_plugin_routes_hash', $routes_hash);
        }
    }
